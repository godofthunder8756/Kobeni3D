...
public class ObjectLoader {
    ...
    private static void processVertex(int pos, int texCoord, int normal, List<Vector2f> texCoordList,
        ...
        if(texCoord >= 0){
            ...
            texCoordArr[pos * 2] = textCoordVec.x;
            texCoordArr[pos * 2 + 1] = 1 - textCoordVec.y;
        }
        if(normal >= 0){
    ...
    public static void processFaces(String token, List<Vector3i>faces){
    ...
    public Model loadModel(float[] vertices, float[] textureCoords, float[] normals, int[] indices){
        ...
        unbind();
        return new Model(id, indices.length); // vertices.length / 3
    }
    ...
    public void storeDataInAttributeList(int attribNo, int vertexCount, float[] data){
        int vbo = GL15.glGenBuffers();
        vbos.add(vbo);
